...
- Day-ahead 48-SP SSP profile forecasts (daily run at 12:30 UTC; weekly HGBR retrain)
...
**Daily pipeline (12:30 UTC, `daily_pipeline.yml`):** Fetches all data sources, builds the feature matrix via `build_features.py`, retrains both HGBR models weekly (level P10/P50/P90, shape H+1 and H+2; age-gated — held frozen between retrains), runs split-conformal PI calibration and writes `pi_calibration_v1.json`, writes the day-ahead 48-SP forecast archive, and commits all artifacts to the `streamlit-data` branch.
...
All three committing workflows (daily, intraday, shadow) can fire concurrently (a daily run at 12:30 UTC overlapping with an intraday update; two concurrent workflow dispatches). Each uses an identical three-attempt retry with autostash:
...
- **Daily-missed threshold:** if `forecast_date < today` at or after 14:00 UTC, the daily run is assumed to have failed.
...
- `daily_pipeline.yml`: GitHub native `schedule: 30 12 * * *` only (no external dispatch). The daily run is more tolerant of timing jitter than the intraday pipeline.
...
| Weekly-retrain consolidation (`forecast_pipeline.yml` → production) | 2-week shadow validation metrics passed (|mae_delta| ≤ £0.50 · |cov_delta| ≤ 3pp for ≥14 consecutive days); weekly age-gated retrain is the production cadence (01:00 UTC consolidated run; CI retirement of `daily_pipeline.yml` pending) | ✅ Completed |
